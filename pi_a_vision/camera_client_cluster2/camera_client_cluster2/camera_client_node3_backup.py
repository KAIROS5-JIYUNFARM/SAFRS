...
#from system_interfaces.msg import SystemMode, EndSignal, Detect
...
# ºÐ·ù¿ë ¸ðµ¨(MobileNet)ÀÇ ÀÔ·Â »çÀÌÁî (º¸Åë 224x224)
...
class CameraNode(Node):
    def __init__(self):
        ...
        self.det_input_h = self.det_in_details[0]['shape'][1]
        self.det_input_w = self.det_in_details[0]['shape'][2]
        ...
        # ÅäÇÈ ¼³Á¤
        self.sub_mode = self.create_subscription(SystemMode, "/mode", self.cb_mode, 10)
        self.pub_error = self.create_publisher(Vector3, "/error_xy", 10)
        self.pub_detect = self.create_publisher(Detect, "/detect", 10)
        self.pub_end = self.create_publisher(EndSignal, "/end", 10)
    ...
    def inference_loop(self):
        while self.running:
            ...
            # ¡Ú ¿¡·¯ ÇØ°á ÇÙ½É: ¸ðµ¨ÀÌ ¿øÇÏ´Â Å©±â·Î ¸®»çÀÌÁî (¿¹: 384x384)
            ...
            if scores[0] > 0.4:
                ...
                self.pub_detect.publish(Detect(label=label, cx=float(cx), cy=float(cy)))
                ...
                if abs(err_x) < self.CENTER_THRESHOLD and conf > 0.8:
                    if self.mode == "STANDBY" or \
                       ...
                       (self.mode == "TRACK_ENEMY" and label == "ENEMY"):
                        self.pub_end.publish(EndSignal(type=label))
            else:
    ...
    def display_loop(self):
        while self.running:
            ...
            if res.get("box"):
                ...
                cv2.putText(frame, f"{res['label']} (B:{res['b_ratio']:.2f})", (x1, y1 - 10), 
